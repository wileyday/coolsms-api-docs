 ...
 */
Gitdown.read = input => {
  let instanceConfig, instanceLogger;
   ...
   */
  gitdown.get = () => {
    return parser.play(input).then(state => {
      let markdown;
      ...
      return gitdown.resolveURLs(markdown).then(() => {
  ...
  gitdown.writeFile = fileName => {
    return gitdown.get().then(outputString => {
  ...
  gitdown.resolveURLs = markdown => {
    ...
    const repositoryURL = gitinfo.compile({ name: 'url' }, { gitdown: gitdown }) + '/tree/' + gitinfo.compile({ name: 'branch' }, { gitdown: gitdown });
 ...
 * @param {number} index
 */
Gitdown.nestHeadingIds.iterateTree = function (tree, callback) {
  let index = arguments.length <= 2 || arguments[2] === undefined ? 1 : arguments[2];
...
//# sourceMappingURL=gitdown.js.map